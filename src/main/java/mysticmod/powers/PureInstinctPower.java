...
public class PureInstinctPower extends AbstractPower implements SpellArteLogicAffector {
    ...
    public PureInstinctPower(AbstractCreature owner, int amount) {
        this.name = NAME;
        this.ID = POWER_ID;
        this.owner = owner;
        this.region128 = new TextureAtlas.AtlasRegion(ImageMaster.loadImage("mysticmod/images/powers/pure instinct power 84.png"), 0, 0, 84, 84);
        this.region48 = new TextureAtlas.AtlasRegion(ImageMaster.loadImage("mysticmod/images/powers/pure instinct power 32.png"), 0, 0, 32, 32);
    ...
    public boolean modifyIsArte(AbstractCard card, boolean isArte) {
        if (card.hasTag(BaseModCardTags.BASIC_STRIKE)) {
            return true;
        }
        return isArte;
    }
